# aave v3 indexer - Pipes SDK 🦑

Stream blockchain data from SQD's Portal, transform it however you need, and send it anywhere. Use the built-ins, write your own, or mix both—everything chains together with `pipe()`.

## Overview

This indexer tracks Aave V3 USDC deposits and withdrawals on Ethereum mainnet, providing real-time metrics on:
- Supply events (USDC deposits → aUSDC minting)
- Withdraw events (aUSDC burning → USDC withdrawals)
